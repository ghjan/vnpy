...
重要提示：
如果你在windows10下，设置了显示放大，例如125%比例，K线和主图指标会显示出错
"""
...
from vnpy.trader.vtConstant import DIRECTION_LONG,DIRECTION_SHORT, OFFSET_OPEN, OFFSET_CLOSE
...
class KLineWidget(KeyWraper):
    ...
    def add_signals(self, df_trade_list):
        ...
        for idx in df_trade_list.index:
            # 开仓时间
            open_time = df_trade_list['OpenTime'].loc[idx]
            if not isinstance(open_time,datetime) and isinstance(open_time,str):
                open_time = datetime.strptime(open_time,'%Y-%m-%d %H:%M:%S')

            open_price = df_trade_list['OpenPrice'].loc[idx]
            direction  = df_trade_list['Direction'].loc[idx]
            if direction == 'Long':
                open_direction  = DIRECTION_LONG
                close_direction = DIRECTION_SHORT
            else:
                open_direction  = DIRECTION_SHORT
                close_direction = DIRECTION_LONG

            close_time = df_trade_list['CloseTime'].loc[idx]
            if not isinstance(close_time, datetime) and isinstance(close_time, str):
                close_time = datetime.strptime(close_time, '%Y-%m-%d %H:%M:%S')

            close_price = df_trade_list['ClosePrice'].loc[idx]
            volume      = df_trade_list['Volume'].loc[idx]

            # 添加开仓信号
            self.add_signal(t_value=open_time, direction=open_direction, offset=OFFSET_OPEN, price=open_price, volume=volume)

            # 添加平仓信号
            self.add_signal(t_value=close_time, direction=close_direction, offset=OFFSET_CLOSE, price=close_price,
                            volume=volume)
    ...
    def add_trans_df(self,df_trans):
        ...
        for idx in df_trans.index:
            ...
            tns_type = DIRECTION_LONG if tns_type== 'long' else DIRECTION_SHORT
    ...
    def loadData(self, df_datas, main_indicators=[], sub_indicators=[]):
        ...
        self.datas = df_datas[['open', 'close', 'low', 'high', 'volume', 'openInterest']].to_records()
        self.axisTime.xdict={}
